 ...
 */
function _findEntityById(worksheet, entityId, idColumnName, headerRowValues) {
    ...
    for (let i = 2; i <= worksheet.rowCount; i++) {
        ...
        const rowValues = row.values; // Can be sparse, first element might be null or match worksheet structure
        ...
        if (cellValue && cellValue.toString().trim() === entityId.trim()) {
            ...
            headerRowValues.forEach((header, index) => {
                if (header) {
                    ...
                    foundEntityData[header.toString().trim()] = rowValues[index];
                }
            });
...
function registerGetExcelTools(server) {
    ...
    server.tool("get_entity_details", getEntityDetailsSchema, async (args) => {
        ...
        try {
            ...
            const headerRowCellValues = worksheet.getRow(1).values;
    ...
    server.tool("get_grouped_stats_summary", getGroupedStatsSummarySchema, async (args) => {
            ...
            for (let i = 2; i <= worksheet.rowCount; i++) {
                ...
                args.statColumnsToAnalyze.forEach(statName => {
                    ...
                    if (numericValue !== null && !isNaN(numericValue) && groupedStats[groupKey][statName].values) {
                        groupedStats[groupKey][statName].values.push(numericValue);
                    }
    ...
    server.tool("get_assembly_details", getAssemblyDetailsSchema, async (args) => {
        ...
        try {
            ...
            if (!assembliesSheet)
                return { isError: true, content: [{ type: "text", text: "Sheet 'AF_Assemblies' not found." }] };
            const assembliesHeader = assembliesSheet.getRow(1).values;
            const assemblyData = _findEntityById(assembliesSheet, args.assemblyId, 'AssemblyID', assembliesHeader);
            if (!assemblyData)
                return { isError: true, content: [{ type: "text", text: `Assembly with ID '${args.assemblyId}' not found.` }] };
            const assemblyName = assemblyData['AssemblyName'] || args.assemblyId;
            let result = { assemblyName };
            const componentPromises = [];
            const componentDetails = {};
            const processEntity = async (entityId, sheetName, idColName, entityType) => {
            ...
            // Frame
            componentPromises.push(processEntity(assemblyData['FrameID'], 'Frames', 'FrameID', 'frame').then(data => {
                if (data && !data.error)
                    componentDetails.frame = data;
                else if (data && data.error)
                    componentDetails.frame = { id: assemblyData['FrameID'], error: data.error };
            }));
            // Pilot
            componentPromises.push(processEntity(assemblyData['PilotID'], 'Pilots', 'PilotID', 'pilot').then(data => {
                if (data && !data.error)
                    componentDetails.pilot = data;
                else if (data && data.error)
                    componentDetails.pilot = { id: assemblyData['PilotID'], error: data.error };
            }));
            // Parts
            const partPromises = [];
            ...
            for (const partType in partIdColumns) {
                const colName = partIdColumns[partType];
                const partId = assemblyData[colName];
                if (partId && partId.trim() !== '') {
                    partPromises.push(processEntity(partId, 'Parts', 'PartID', `part (${partType})`).then(data => {
                        if (data && !data.error)
                            componentDetails.parts[partType] = data;
                        else if (data && data.error)
                            componentDetails.parts[partType] = { id: partId, error: data.error };
                    }));
                }
            }
            // 팔 파츠 처리 (좌/우 구분)
            const leftArmId = assemblyData['LeftArmPartID'];
            if (leftArmId && leftArmId.trim() !== '') {
                partPromises.push(processEntity(leftArmId, 'Parts', 'PartID', 'part (leftarm)').then(data => {
                    if (data && !data.error)
                        componentDetails.parts['leftarm'] = data;
                    else if (data && data.error)
                        componentDetails.parts['leftarm'] = { id: leftArmId, error: data.error };
                }));
            }
            const rightArmId = assemblyData['RightArmPartID'];
            if (rightArmId && rightArmId.trim() !== '') {
                partPromises.push(processEntity(rightArmId, 'Parts', 'PartID', 'part (rightarm)').then(data => {
                    if (data && !data.error)
                        componentDetails.parts['rightarm'] = data;
                    else if (data && data.error)
                        componentDetails.parts['rightarm'] = { id: rightArmId, error: data.error };
                }));
            }
            // 무기 처리
            for (const weaponSlot in weaponIdColumns) {
                const colName = weaponIdColumns[weaponSlot];
                const weaponId = assemblyData[colName];
                if (weaponId && weaponId.trim() !== '') {
                    partPromises.push(processEntity(weaponId, 'Weapons', 'WeaponID', `weapon (${weaponSlot})`).then(data => {
                        ...
                        if (data && !data.error)
                            componentDetails.parts[weaponSlot] = data;
                        else if (data && data.error)
                            componentDetails.parts[weaponSlot] = { id: weaponId, error: data.error };
                    }));
                }
            }
            ...
            const accumulateStats = (component) => {
                ...
                for (const key in component) {
                    if (key.startsWith('Stat_')) {
                        ...
                        if (statValue !== null && typeof statValue !== 'undefined') {
                            ...
                            else if (typeof statValue === 'object' && statValue.richText) {
                                // Handle RichText case if necessary, though stats are usually not RichText
                                let textContent = "";
                                statValue.richText.forEach(rt => textContent += rt.text);
                        ...
                        if (typeof abilitiesValue === 'string' && abilitiesValue.trim() !== '') {
                            abilitiesValue.split(',').forEach((ability) => allAbilities.add(ability.trim()));
                        }
                    }
            ...
            Object.values(componentDetails.parts).forEach((partValue) => {
                const part = partValue; // Cast to the more specific type
                // Check if 'error' property does NOT exist or is falsy. Also check if 'id' exists for the error case.
                if (part && !part.error) {
                    accumulateStats(part);
                }
                else if (part && part.id && part.error) {
                    console.warn(`[ExcelGetTools] Skipping stats accumulation for part ID '${part.id}' due to error: ${part.error}`);
                }
            });
