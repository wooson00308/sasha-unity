            ...
            weaponIdColumns.forEach(colName => {
                ...
                if (weaponId && weaponId.trim() !== '') {
                    ...
                    partPromises.push(processEntity(weaponId, 'Weapons', 'WeaponID', `weapon (${weaponSlot})`).then(data => {
                        if (data && !data.error)
                            componentDetails.parts[weaponSlot] = data; // Store under parts for now, might need a separate weapons category
                        else if (data && data.error)
            ...
            const accumulateStats = (component) => {
                ...
                for (const key in component) {
                    if (key.startsWith('Stat_') && typeof component[key] === 'number') {
                        ...
                        component[key].split(',').forEach((ability) => allAbilities.add(ability.trim()));
                    }
