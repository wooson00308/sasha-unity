...
-   **주요 기능**:
    ...
    -   **로그 처리 및 재생**: 전투 종료 (`CombatEndEvent` evt) 시, 이벤트에 포함된 `evt.FinalLog` (`List<LogEntry>`)를 사용합니다. 각 로그 항목을 순회하며 상태 스냅샷과 델타 정보를 사용하여 특정 시점의 게임 상태를 복원(`UpdateSnapshotWithDelta`)하고, 이를 바탕으로 유닛 상세 정보 UI(`_unitDetailTextDisplay` 등)를 업데이트합니다. 로그 메시지는 `FlavorTextSO`를 참조하여 가공된 후, UI 텍스트 요소(`_logLinePrefab` 인스턴스)에 타이핑 애니메이션 효과와 함께 순차적으로 표시됩니다.
        -   **애니메이션 속도 커스터마이징**: `LogEventType`별로 텍스트 라인 표시 딜레이 (`lineDelay`) 및 글자당 타이핑 시간 (`durationPerCharacter`), 최소/최대 애니메이션 시간 (`minMaxTextAnimationDuration`)을 Odin Inspector를 통해 `Dictionary<LogEventType, LogSpeedParameters>` 형태로 설정할 수 있습니다. 특정 `LogEventType`에 대한 설정이 없을 경우, `_defaultLineDelay`, `_defaultDurationPerCharacter`, `_defaultMinMaxTextAnimationDuration` 등의 기본값이 사용됩니다. 이 설정은 `SerializedMonoBehaviour`를 상속하고 `[DictionaryDrawerSettings]` 어트리뷰트를 활용하여 인스펙터에서 편리하게 관리됩니다.
