...
namespace AF.Combat.Handlers
{
    public class DefenseReducedHandler : IStatusEffectHandler // Debuff용
    {
        public void OnApply(CombatContext ctx, ArmoredFrame target, StatusEffect effect)
        {
            ...
            ctx.Logger.TextLogger.Log($"<color=#00FFFF>{target.Name}</color> 기체 장갑 <color=red>약화</color> 발생. (효과: {effect.ModificationValue:P0}, 지속시간: {effect.DurationTurns}턴)", LogLevel.Info, LogEventType.StatusEffectApplied);
        }
        ...
        {
            ctx.Logger.TextLogger.Log($"<color=#00FFFF>{target.Name}</color> 기체 장갑 약화 효과 <color=yellow>소멸</color>.", LogLevel.Info, LogEventType.StatusEffectExpired);
        }
        public void OnRemove(CombatContext ctx, ArmoredFrame target, StatusEffect effect)
        {
            ...
            ctx.Logger.TextLogger.Log($"<color=#00FFFF>{target.Name}</color> 기체 장갑 약화 효과 <color=green>복구</color>.", LogLevel.Info, LogEventType.StatusEffectRemoved);
        }
    }
} 