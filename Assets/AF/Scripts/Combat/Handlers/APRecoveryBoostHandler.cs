...
namespace AF.Combat.Handlers
{
    public class APRecoveryBoostHandler : IStatusEffectHandler
    {
        public void OnApply(CombatContext ctx, ArmoredFrame target, StatusEffect effect)
        {
            ...
            ctx.Logger.TextLogger.Log($"<color=#00FFFF>{target.Pilot.Name}</color> 기체 에너지 회복 시스템 <color=green>최적화</color>. (AP 회복량 증가: {effect.ModificationValue}, 지속시간: {effect.DurationTurns}턴)", LogLevel.Info, LogEventType.StatusEffectApplied);
        }
        ...
        {
            ctx.Logger.TextLogger.Log($"<color=#00FFFF>{target.Pilot.Name}</color> 기체 에너지 회복 최적화 효과 <color=yellow>만료</color>.", LogLevel.Info, LogEventType.StatusEffectExpired);
        }
        ...
        public void OnRemove(CombatContext ctx, ArmoredFrame target, StatusEffect effect)
        {
            ...
            ctx.Logger.TextLogger.Log($"<color=#00FFFF>{target.Pilot.Name}</color> 기체 에너지 회복 시스템 <color=red>원상 복구</color>.", LogLevel.Info, LogEventType.StatusEffectRemoved);
        }
    }
} 