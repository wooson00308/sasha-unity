...
namespace AF.Combat.Handlers
{
    ...
    public class DefenseBoostHandler : IStatusEffectHandler
    {
        public void OnApply(CombatContext ctx, ArmoredFrame target, StatusEffect effect)
        {
            ...
            ctx.Logger.TextLogger.Log($"<color=#00FFFF>{target.Pilot.Name}</color> 기체 장갑 <color=green>강화</color>. 방어력 <color=green>상승</color>. (효과: {effect.ModificationValue:P0}, 지속시간: {effect.DurationTurns}턴)", LogLevel.Info, LogEventType.StatusEffectApplied);
        }
        ...
        {
            ctx.Logger.TextLogger.Log($"<color=#00FFFF>{target.Pilot.Name}</color> 기체 장갑 강화 효과 <color=yellow>만료</color>.", LogLevel.Info, LogEventType.StatusEffectExpired);
        }
        ...
        public void OnRemove(CombatContext ctx, ArmoredFrame target, StatusEffect effect)
        {
            ...
            ctx.Logger.TextLogger.Log($"<color=#00FFFF>{target.Pilot.Name}</color> 기체 장갑 강화 효과 <color=red>해제</color>.", LogLevel.Info, LogEventType.StatusEffectRemoved);
        }
    }
} 