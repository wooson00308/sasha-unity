...
namespace AF.Combat.Handlers
{
    ...
    public class RepairOverTimeHandler : IStatusEffectHandler
    {
        ...
        {
            ctx.Logger.TextLogger.Log($"<color=#00FFFF>{target.Pilot.Name}</color> 기체에 <color=green>나노 수리봇</color> 활성화. (틱당 회복량: {effect.TickValue}, 지속시간: {effect.DurationTurns}턴)", LogLevel.Info, LogEventType.StatusEffectApplied);
        }
        ...
        public void OnTick(CombatContext ctx, ArmoredFrame target, StatusEffect effect)
        {
            ...
            if (actualRepairedAmount > 0)
            {
                ...
                ctx.Logger.TextLogger.Log($"<color=#00FFFF>{target.Pilot.Name}</color> 나노 수리봇 작동: 기체 내구도 <color=green>{actualRepairedAmount} 회복</color>.", LogLevel.Info, LogEventType.StatusEffectTicked);
            }
        }

        public void OnExpire(CombatContext ctx, ArmoredFrame target, StatusEffect effect)
        {
            ctx.Logger.TextLogger.Log($"<color=#00FFFF>{target.Pilot.Name}</color> 기체 나노 수리봇 활동 <color=yellow>종료</color>.", LogLevel.Info, LogEventType.StatusEffectExpired);
        }

        public void OnRemove(CombatContext ctx, ArmoredFrame target, StatusEffect effect)
        {
            ctx.Logger.TextLogger.Log($"<color=#00FFFF>{target.Pilot.Name}</color> 기체 나노 수리봇 <color=red>강제 비활성화</color>.", LogLevel.Info, LogEventType.StatusEffectRemoved);
        }
    }
} 