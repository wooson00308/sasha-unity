            ...
            DetermineAction(ArmoredFrame activeUnit, CombatSimulatorService ctx)
        {
            ...
            if (closestOperationalEnemy != null) // Check target validity
            {
                ...
                if (meleeWeapon != null)
                {
                    ...
                    bool canAttack = minDist <= meleeWeapon.Range.MaxRange + 0.001f && // 이미 사거리 내인가? (약간의 오차 허용)
            ...
            if (canMove && closestOperationalEnemy != null)
            {
                ...
                else // 아직 이동 안 했으면 이동 로직 실행
                {
                    ...
                    float targetDistance = meleeWeaponForRange != null ? meleeWeaponForRange.Range.MaxRange * 0.5f : 0.1f; // 무기 최대 사거리의 절반 또는 최소 거리 목표
